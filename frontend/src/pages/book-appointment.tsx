...
/**
 * Reverted to original BookAppointment UI as per user request.
 * Kept the extracted useAppointmentBooking hook for business logic.
...
const BookAppointment: React.FC = () => {
    ...
    return (
        <main className="flex-1 flex flex-col items-center py-8 px-4 lg:px-20 font-display min-h-screen">
            <div className="w-full max-w-[1000px] flex flex-col gap-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-primary/10 shadow-sm">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <div className="flex items-center gap-4 bg-primary/5 rounded-lg p-1">
                                    <button onClick={() => actions.shiftMonth(-1)} className="p-2 hover:bg-white rounded-md shadow-sm transition-all"><span className="material-symbols-outlined text-sm">chevron_left</span></button>
                                    <span className="font-bold text-sm min-w-[120px] text-center uppercase tracking-wider text-primary">{MONTHS[viewDate.month]} {viewDate.year}</span>
                                    <button onClick={() => actions.shiftMonth(1)} className="p-2 hover:bg-white rounded-md shadow-sm transition-all"><span className="material-symbols-outlined text-sm">chevron_right</span></button>
                                </div>
                            </div>
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-primary/10 shadow-sm overflow-hidden text-center">
                            ...
                            <div className="pt-12 px-6 pb-6">
                                <h4 className="font-bold text-xl text-primary dark:text-white">{targetDoctor.fullName}</h4>
                                ...
                                <div className="border-t border-slate-100 dark:border-slate-800 pt-6 space-y-4 text-left text-sm">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-primary dark:text-white font-bold">{selectedTimeSlot}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-slate-500">Consultation Fee</span>
                                        <span className="text-primary dark:text-white font-bold">${targetDoctor.consultationFee || '120.00'}</span>
                                    </div>
                                </div>
                                <button
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm p-8 relative" onClick={e => e.stopPropagation()}>
                        ...
                        {isBookingConfirmed ? (
                            <div className="text-center py-4">
                                ...
                                <p className="text-slate-500 dark:text-slate-400 text-sm">You will receive a confirmation email and calendar invite. Redirecting to dashboard...</p>
                            </div>
                        ) : (
